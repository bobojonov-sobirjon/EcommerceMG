...
| `price_on_request` | Что показывать на фронте |
|--------------------|--------------------------|
| `true` | **«Цена по запросу»** (не выводить `price` как сумму) |
| `false` | Обычная цена из `price` |

При `price_on_request: true` значение `price` в API обычно `"0.00"` — это техническое, не показывайте пользователю.
